...
export async function registerRoutes(
  ...
  app.patch("/api/avaliacoes/:id", autenticar, async (req: any, res) => {
    try {
      const id = Number(req.params.id);
      const data = req.body;
      if (data.notaMaxima) data.notaMaxima = parseFloat(data.notaMaxima);
      if (data.peso) data.peso = parseFloat(data.peso);
      const updated = await storage.atualizarAvaliacao(id, data);
      res.json(updated);
    } catch (err: any) {
      res.status(400).json({ mensagem: "Erro ao atualizar avaliação", detalhes: err.message });
    }
  });

  app.patch("/api/unidades-curriculares/:id", autenticar, async (req: any, res) => {
    try {
      const id = Number(req.params.id);
      const updated = await storage.atualizarUnidadeCurricular(id, req.body);
      res.json(updated);
    } catch (err: any) {
      res.status(400).json({ mensagem: "Erro ao atualizar unidade curricular" });
    }
  });

  app.delete("/api/unidades-curriculares/:id", autenticar, async (req: any, res) => {
    try {
      const id = Number(req.params.id);
      await storage.excluirUnidadeCurricular(id);
      res.status(204).end();
    } catch (err: any) {
      res.status(400).json({ mensagem: "Erro ao excluir unidade curricular" });
    }
  });
  ...
  // Notas
  app.get("/api/turmas/:id/notas", autenticar, async (req, res) => {
    const turmaId = Number(req.params.id);
    const result = await storage.getNotasDaTurma(turmaId);
    res.json(result);
  });
